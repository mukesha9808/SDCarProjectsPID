    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
           ...
           */
          // saturate lower speed value to 10mph
          if(speed <10)
            speed=10;
          
          //Get new modified error term
          double cte_modified=cte/speed;
          
          //Update PID errors
          pid_steer.UpdateError(cte_modified);
          ...
          //Update speed PID error terms
          pid_speed.UpdateError(-(50-speed));
          
          //Get throttle value
          double thrt=pid_speed.TotalError();
          if  (thrt> 1) {
            thrt=1;
          }
          else if( thrt <0){
